document.addEventListener("DOMContentLoaded", () => {
  ...
  if(heroForm){
    ...
    heroForm.addEventListener("submit",(e)=>{
      ...
      if(!email.includes("@")) return alert("ادخلي ايميل صحيح");
      alert("تم استلام عنوان البريد — شكراً!");
  ...
  if(orderForm){
    orderForm.addEventListener("submit", async(e)=>{
      ...
      } catch(err){ resultEl.style.color="crimson"; resultEl.textContent="فشل بالاتصال بالخادم"; console.error(err);}
    });
    ...
    async function fetchOrders(){
      ...
      try{
        ...
        renderOrders(arr||[]);
      } catch(err){ ordersList.innerHTML="<p style='color:gray'>تعذر جلب الطلبات</p>"; console.error(err);}
    }

    function renderOrders(list){
      if(!list || list.length===0){ordersList.innerHTML="<p style='color:gray'>لا توجد طلبات بعد</p>";return;}
